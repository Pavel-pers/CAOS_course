...
На стандартный поток вывода напечатайте ключи, размещенные в данном дереве, в порядке обхода in-order (для каждого поддерева сначала выводятся все ключи левого поддерева, потом корень, потом ключи правого поддерева).

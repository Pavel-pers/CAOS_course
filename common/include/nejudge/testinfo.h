...
{
    char * (*substitute)(struct testinfo_subst_handler *, const char *);
};
...
char *testinfo_unparse_cmdline(const testinfo_t *pt);
char *testinfo_unparse_environ(const struct testinfo_struct *ti);
...
read_line(FILE *fin, struct line_buf *pbuf)
{
    ...
    if (!pbuf->a || !pbuf->v) {
        pbuf->a = 512;
        if (!(pbuf->v = (char*) malloc(pbuf->a))) goto failure;
    }
    pbuf->u = 0;
    pbuf->v[0] = 0;
    ...
    while (1) {
        ...
        if (pbuf->u + 1 >= pbuf->a) {
            pbuf->a *= 2;
            old_v = pbuf->v;
            if (!(pbuf->v = (char*) realloc(pbuf->v, pbuf->a))) {
                pbuf->v = old_v;
                goto failure;
            }
        }
...
static int
parse_cmdline(const char *str, struct cmdline_buf *pcmd)
{
    char *locbuf, *q, *qq, **old_v;
    const char *p = str;
    char nb[4];
    int q_char = 0;
    int code, i;
    ...
    pcmd->u = 0;
    pcmd->v[0] = 0;
    if (!(q = locbuf = (char*) alloca(strlen(str) + 16))) {
        code = -TINF_E_NO_MEMORY;
        goto failure;
    }
    while (isspace(*p)) p++;
...
static void
append_char(char **p_t, int *p_a, int *p_u, int c)
{
    if (!*p_t || !*p_a) {
        *p_a = 32;
        *p_t = (char*) malloc(*p_a);
...
static void
append_string(char **p_t, int *p_a, int *p_u, const char *s)
{
    int slen;
...
static void
append_string_quoted(char **p_t, int *p_a, int *p_u, const char *s)
{
    char buf[16];
    if (!s || !*s) return;
    for (; *s; ++s) {
...
static int
parse_size(const char *str, long long *p_value)
{
    if (!str || !*str) return -1;
    char *eptr = NULL;
    errno = 0;
...
static int
parse_line(const char *str, size_t len, testinfo_t *pt, struct testinfo_subst_handler *sh)
{
    char *subst_str = NULL;
    const char *s = str;
    char *name_buf = 0, *p;
    ...
    if (!(name_buf = (char *) alloca(len + 1))) FAIL(TINF_E_NO_MEMORY);
    if (!(val_buf = (char *) alloca(len + 2))) FAIL(TINF_E_NO_MEMORY);
    ...
    *p = 0;
    ...
    if (!strcmp(name_buf, "params")) {
        ...
        if (!strcmp(name_buf, "comment")) {
            ppval = (char**) ((void*) &pt->comment);
        } else if (!strcmp(name_buf, "source_stub")) {
            ppval = (char**) ((void*) &pt->source_stub);
        } else if (!strcmp(name_buf, "working_dir")) {
            ppval = (char**) ((void*)&pt->working_dir);
        } else if (!strcmp(name_buf, "program_name")) {
            ppval = (char**) ((void*)&pt->program_name);
        } else {
            ppval = (char**) ((void*)&pt->team_comment);
        }
